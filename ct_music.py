...
def _phoneticize_number(num: int) -> Tuple[int, int]:
    if num == 0:
        ...
        raise MusicError(
            f'Can only phoneticize numbers between 0-59, inclusive. Got: {num}')


def play_sequence(sequence: Sequence, on_func: Callable[[int], None],
                  off_func: Callable[[int], None], solenoid_on_time):
...
def current_phonetic_time(now=None):
    ...
    minutes = sum([_time_sequences[n]
                  for n in _phoneticize_number(now.tm_min)], [])
